...
\title{\texttt{getSeq} \input{version}: \emph{Description}}
\author{\input{author}}
\input{date}
...
The program \texttt{getSeq} was written in C on a computer running Linux.
Please contact \texttt{\input{email}} if there are any problems
with the program.
\begin{itemize}
\item Obtain the package\\
\texttt{git clone https://www.github.com/\input{repo}/getSeq}
\item Change into the directory just downloaded
\begin{verbatim}
cd getSeq
\end{verbatim}
and make \texttt{getSeq}
\begin{verbatim}
make
\end{verbatim}
\item Test \texttt{getSeq}
\begin{verbatim}
make test
\end{verbatim}
\item The executable \texttt{getSeq} is located in the
